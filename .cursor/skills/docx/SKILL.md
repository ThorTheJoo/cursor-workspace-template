...
```javascript
// docx-js defaults to A4, not US Letter - always set page size explicitly
sections: [{
  properties: {
    page: {
      size: {
...
- **Never use tables as dividers/rules** — cells have minimum height; use border on Paragraph instead
...
Edit files in `unpacked/word/`. Use "Claude" as the author for tracked changes and comments, unless the user requests otherwise.
...
- **Replace entire run elements**: When adding tracked changes, replace the whole run block with insertion/deletion as siblings. Don't inject tracked change tags inside a run.
- **Preserve run formatting**: Copy the original run's formatting block into your tracked change runs to maintain bold, font size, etc.
...
4. Reference in document.xml with `<w:drawing>` element
